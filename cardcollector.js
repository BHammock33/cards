    ...
    try {
        ...
        if(onlyRares.checked){
            for (var i = 0; i < cards.length; i++) {
                cardTable.innerHTML += "<tr><td>" + rareCards[i].id + "</td><td>" + rareCards[i].name + "</td><td>"+ currencySymbol + Math.round((rareCards[i].priceUSD * priceMultiplier) * 100) /100 + "</td><td>" + rareCards[i].isRare + "</td></tr>"
            }
        }else{
            for (var i = 0; i < cards.length; i++) {
                cardTable.innerHTML += "<tr><td>" + cards[i].id + "</td><td>" + cards[i].name + "</td><td>"+ currencySymbol + Math.round((cards[i].priceUSD * priceMultiplier) * 100)/100 + "</td><td>" + cards[i].isRare + "</td></tr>"
            }
        }
